...
describe('<CardBack />', () => {
	...
	const cardBack = shallow(
		<CardBack title="Rolling in the Deep" genres={genres} IMDBRating={3} />
	);
	...
	it("renders 'No Rating Found' in an <h4> if the IMDBRating prop is null", () => {
		const nullRating = shallow(
			<CardBack title="Rolling in the Deep" genres={['s']} IMDBRating={null} />
		);
		expect(
			nullRating
				.find('h4')
